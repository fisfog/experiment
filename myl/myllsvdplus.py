...
class LSVDPlus():
	def __init__(self,r2=2,alpha=0.005,beta=0.02,sr=0.99,iters=60):
		self.alpha =  alpha
		self.beta = beta
		self.beta2 = 1
		self.beta3 = 1
	...
	def pred(self,u,i):
		return self.mean+self.b_u[u]+self.b_i[i]+np.dot(self.t_u[u],self.ldatheta[i])\
	...
	def sgd(self):
		...
		for step in xrange(self.max_iter):
			...
			for k in xrange(self.train_record):
				...
				self.t_u[u] += self.alpha*(eui*self.ldatheta[i]-self.beta*self.t_u[i])
