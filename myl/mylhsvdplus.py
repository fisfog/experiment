...
class HSVDPlus():
	def __init__(self,r2=2,alpha=0.005,beta0=0.5,beta1=0.02,sr=0.99,iters=60,l1=1,l2=2):
		self.alpha = alpha
		self.r2 = r2
		self.beta0 = beta0
		self.beta1 = beta1
	...
	def initialize_model(self,metadata,ldatheta):
		...
		self.r1 = self.ldatheta.shape[1]
		self.dim = self.r1+self.r2
		...
		self.t_u = np.random.random((self.N,self.r1))
		self.p_u = np.random.random((self.N,self.r2))
		self.q_i = np.random.random((self.M,self.r2))
	...
	def _update_ti(self):
		self.t_i = np.zeros((self.M,self.r1))
		for i in xrange(self.M):
			for j in xrange(len(self.w[i])):
				self.t_i[i] += self.w[i][j]*self.theta[i][j]

	def pred(self,u,i):
		return self.mean+self.b_u[u]+self.b_i[i]+np.dot(self.t_u[u],self.t_i[i])\
			+np.dot(self.p_u[u],self.q_i[i])
	...
	def sgd(self):
		...
		for step in xrange(self.max_iter):
			...
			for k in xrange(self.train_record):
				...
				self.t_i[i] = np.array([0 for i in xrange(self.r1)])
				for j in xrange(len(self.w[i])):
					self.w[i][j] += self.alpha*(eui*np.dot(self.t_u[u],self.theta[i][j])+\
						self.beta0*(self.h[i][j]-self.w[i][j])+self.beta*self.w[i][j])
					self.t_i[i] += self.w[i][j]*self.theta[i][j]
				self.t_u[u] += self.alpha*(eui*self.t_i[i]-self.beta1*self.t_u[u])
				self.q_i[i] += self.alpha*(eui*self.p_u[u]-self.beta1*self.q_i[i])
